...
const PENDING_HEADERS = {
  ...
  followup_status:     ['案件狀態', '後續追蹤', '後續追蹤狀態', '跟進狀態', '追蹤狀態', '狀態'],
  client_name:         ['當事人', '客戶', '客戶名稱', '案主', '當事人姓名'],
  is_paid:             ['已付款', '已付', '是否已付'],
  channel:             ['管道來源', '來源', '案源', '渠道', '管道', '客戶來源', '案件來源'],
  consultation_lawyer: ['律師諮詢', '諮詢律師', '接洽律師', '律師'],
  last_contact_text:   ['最近一次聯繫時間', '最近聯繫', '最後聯繫', '最後聯繫時間', '最近一次聯繫', '聯繫'],
  case_summary:        ['簡述案件', '案件摘要', '案情', '案件描述', '案件簡述', '簡述', '摘要'],
  first_contact_at:    ['初次接觸', '初次接觸日', '首次接觸', '首次接觸日', '初次聯繫日'],
  cm_meeting_at:       ['客戶經理與當事人第一次會議日期', 'cm會議', '客戶經理會議', '客經會議', 'cm會議日'],
  lawyer_meeting_at:   ['律師會議', '律師會', '律師會議日', '律師會議日期'],
  proposal_at:         ['提報方案日期', '提報方案', '提報日', '提案日', '提案', '方案提報'],
  follow_up_1_at:      ['提報後追蹤日期', '提報追蹤', '第一次追蹤', '追蹤一', '追蹤1', '一次追蹤'],
  follow_up_2_at:      ['第二次追蹤', '追蹤二', '追蹤2', '二次追蹤'],
  lawyer_notes:        ['備註', '律師備註', '備註欄', '律師協助'],
  is_signed:           ['簽約', '已成案', '是否成案', '已簽約', '成案'],
  payment_status:      ['付款', '付款狀態', '收款狀態', '繳費狀態']
};
...
function buildPendingColMap(headerRow) {
  // normalize：把「(請手填xxxx/xx/xx)」「（會議後三天內）」這類括號註解整段拔掉、
  // 拿掉空白、轉小寫，方便比對。
  const norm = s => String(s == null ? '' : s)
    .replace(/[（(][^）)]*[）)]/g, '')   // 半形/全形括號連同內容一起拔掉
    .replace(/[\s　]+/g, '')              // 拿掉殘留空白
    .toLowerCase();

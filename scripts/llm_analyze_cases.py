...
PROMPT_VERSION = "v1.2"  # v1.2: 加術語規範（禁用「包價」、區分逐字稿引用與敘述用語）
...
**術語規範**（這是台灣律師事務所，請使用本所慣用詞）：
- **禁用「包價」這個詞** — 台灣法律業沒有「包價」用法。要表達「整案計費」用「打包價」「整案報價」「一站式報價」；要表達「給客戶費用」用「報價」
- **「老師」只能出現在 reason_evidence 的引號內**（逐字稿口誤可如實引用）— 在 missed_opportunities / improvement_for_lawyer / transferable_pattern 等敘述欄位描述律師助理時一律用「同事」，不要寫「請同事 / 老師…」這種並列
- 律師事務所的內部分工角色：律師、合署律師、所長、（接案）法務同仁、業務、客戶經理 — 沒有「老師」這個 role
"""

-- v5: 強化 dedup partition key — Section B 重複資料把 intake_date 設為 NULL，
-- v4 partition key 還包含 intake_date 所以漏抓部分重複。
-- 改成：partition by (handling_lawyer, team_owner, referral_year/month,
--                    first_payment_date, first_payment_amount, case_amount,
--                    attended, signed, channel)
-- 不含 intake_date — 而是用 (attended, signed, channel) 補足區辨力。
-- 已用 4月 raw data 模擬驗證：v5 vs sheet 每位律師 100% 對齊（4月當月 合計 3,654,500）。
...
COMMENT ON FUNCTION rebuild_fact_010_monthly_team IS 'v5: partition key 不含 intake_date (Section B 副本把 intake 設為 NULL)，改用 attended/signed/channel 區辨';
COMMENT ON FUNCTION rebuild_fact_010_monthly_lawyer IS 'v5: 同上';
